...
def IFFT_GI(symbol_stream, plot=False):
    ...
    for i in range(num_symbols):
        ...
        if plot:
            ...
            plt.title("Podaci na pozicijama")
            plt.grid(True)
            plt.show(block=False)
        ...
        if plot:
            ...
            plt.title("Samo piloti na pozicijama")
            plt.grid(True)
            plt.show(block=False)
        ...
        if plot:
            ...
            plt.subplot(2,1,2)
            plt.stem(np.imag(IFFT_input))
            plt.title("Cijeli OFDM signal u frekvencijskoj domeni")
            plt.grid(True)
            plt.show(block=False)
