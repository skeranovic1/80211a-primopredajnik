...
def main():
    ...
    #Pretprocesing (downsample -> fs postaje 20 MHz)
    rx_signal, fs1 = iq_preprocessing(rx_signal, tx_signal, fs)
    ...
    t_us = np.arange(len(rx_signal)) / fs1 * 1e6
    plt.figure(figsize=(12, 4))
    plt.plot(t_us, np.real(rx_signal), label="RX signal (real)")
    plt.axvline(fft_start / fs1 * 1e6, color="red", linestyle="--",label="Detektovani početak (fft_start)")
    ...
    #Plot 2: timing korelacija + marker
    plt.figure(figsize=(12, 4))
